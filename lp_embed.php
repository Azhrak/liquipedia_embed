        ...
        <tr>
          <th colspan="4" class="group_name">
            ...
            <?php if (!empty($time)) : ?>
              ...
              <span class="group_time"><?php echo preg_replace('/[^öä\w\d ,:\.]/', '', $time); //date("j.n, H:i", $group['time']);?></span>
            <?php endif; ?>
          </th>
        </tr>
  ...
  for ($k=0; $k<count($offsets); $k++) {
    ...
    if (preg_match('/div class="bracket-column"/', $html_slice)) { // New bracket format, with DIVs
      ...
      for ($i=0; $i<count($matches[0]); $i++) {
        ...
        // if (!empty($race)) {
        preg_match('/src="[^"]+?\/([\w]{2})\.\w{3}"/', $match, $hit);
      ...
      for ($i=0; $i<count($matches[0]); $i++) {
        ...
        if (strtoupper($color) == 'F2F2F2') { // score cell
          ...
          switch($color) {
            case 'F2B8B8' : $race = 'Zerg'; break;
            case 'B8B8F2' : $race = 'Terran'; break;
            case 'B8F2B8' : $race = 'Protoss'; break;
          }
...
function round_name($round) {
  switch ($round) {
    case 2        : return FINAL_MATCH;
    case 4        : return SEMIFINAL_MATCH;
    case 'bronze' : return BRONZE_MATCH;
  }
  return "RO".$round;
}
...
function race_icon($race) {
  switch ($race) {
    case 'Protoss' : return SMILIES_DIR.IMG_PROTOSS;
    case 'Terran'  : return SMILIES_DIR.IMG_TERRAN;
    case 'Zerg'    : return SMILIES_DIR.IMG_ZERG;
    case 'Random'  : return SMILIES_DIR.IMG_RANDOM;
  }
  return '';
}
